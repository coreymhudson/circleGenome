...
(function( globalName, root, factory ){
	...
	function define_Please(){
		...
		function random_seed_int( min, max, seed ){
			return Math.floor(random_seed(seed) * (max - min + 1 )) + min;
		}

		function random_seed_float( min, max, seed ){
			return random_seed(seed) * ( max - min ) + min;
		}
		...
		Please.make_color = function( options ){
			...
			for ( var i = 0; i < color_options.colors_returned; i++ ) {
				var random_hue;
				if ( color_options.seed != null) {
					random_hue = (random_seed_int( 0, 360, color_options.seed));
					color_options.seed = random_hue;
				}
				else {
					random_hue = random_int( 0, 360 );
				}
				var hue,saturation,value;
				if( base_color != null ){
					if ( color_options.seed != null) {
						hue = random_seed_int( ( base_color.h - 5 ), ( base_color.h + 5 ), color_options.seed);
						saturation = random_seed_float( .4, .85, color_options.seed );
						color_options.seed = hue;
						value = random_seed_float( .4, .85, color_options.seed );
					} 
					else {
